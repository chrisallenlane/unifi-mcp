...
build: | clean $(dist_dir) fmt lint vet
	$(GO) build $(BUILD_FLAGS) -o $(dist_dir)/unifi-mcp-server $(cmd_dir)
...
# unifi-mcp-server-darwin-amd64
$(dist_dir)/$(bin)-darwin-amd64:
	GOARCH=amd64 GOOS=darwin \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-darwin-arm64
$(dist_dir)/$(bin)-darwin-arm64:
	GOARCH=arm64 GOOS=darwin \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-linux-386
$(dist_dir)/$(bin)-linux-386:
	GOARCH=386 GOOS=linux \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-linux-amd64
$(dist_dir)/$(bin)-linux-amd64:
	GOARCH=amd64 GOOS=linux \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-linux-arm5
$(dist_dir)/$(bin)-linux-arm5:
	GOARCH=arm GOOS=linux GOARM=5 \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-linux-arm6
$(dist_dir)/$(bin)-linux-arm6:
	GOARCH=arm GOOS=linux GOARM=6 \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-linux-arm7
$(dist_dir)/$(bin)-linux-arm7:
	GOARCH=arm GOOS=linux GOARM=7 \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-linux-arm64
$(dist_dir)/$(bin)-linux-arm64:
	GOARCH=arm64 GOOS=linux \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-netbsd-amd64
$(dist_dir)/$(bin)-netbsd-amd64:
	GOARCH=amd64 GOOS=netbsd \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-openbsd-amd64
$(dist_dir)/$(bin)-openbsd-amd64:
	GOARCH=amd64 GOOS=openbsd \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-solaris-amd64
$(dist_dir)/$(bin)-solaris-amd64:
	GOARCH=amd64 GOOS=solaris \
	$(GO) build $(BUILD_FLAGS) -o $@ $(cmd_dir) && $(GZIP) $@ && chmod -x $@.gz

# unifi-mcp-server-windows-amd64
$(dist_dir)/$(bin)-windows-amd64.exe:
...
## install: build and install unifi-mcp-server on your PATH
.PHONY: install
install: build
